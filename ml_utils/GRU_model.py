...
class GRU(nn.Module):
    ...
    def __init__(self, in_chans, hidden_size1, num_layers1, hidden_size2, num_layers2, num_classes, dropout, bidirectional, pre_out, single_layer, linear_size, use_layernorm, batch_size, device1):
        ...
        ##Use these two for two seperate GRU layers
        self.GRU = nn.GRU(self.in_chans, self.hidden_size1, self.num_layers1, batch_first=True, bidirectional = self.bidirectional) #uncomment for first GRU layer

        #self.GRU = nn.GRU(input_size, hidden_size, num_layers, batch_first=True, bidirectional = bidirectional) 
        self.GRU2 = nn.GRU(self.hidden_size1*self.bi_mult, self.hidden_size2, self.num_layers2, batch_first=True, bidirectional = self.bidirectional)
